...
test('router.createPlainTransport() succeeds', async () =>
{
	...
	await expect(ctx.router!.dump())
		.resolves
		.toMatchObject({ transportIds: [ plainTransport.id ] });
	...
	const anotherTransport =
		await ctx.router!.createPlainTransport({ listenIp: '127.0.0.1' });
...
test('router.createPlainTransport() with wrong arguments rejects with TypeError', async () =>
{
	...
	await expect(ctx.router!.createPlainTransport({ listenIp: '123' }))
		.rejects
		.toThrow(TypeError);

	// @ts-ignore
	await expect(ctx.router!.createPlainTransport({ listenIp: [ '127.0.0.1' ] }))
		.rejects
		.toThrow(TypeError);
...
{
	await expect(ctx.router!.createPlainTransport({ listenIp: '8.8.8.8' }))
		.rejects
		.toThrow(Error);
}, 2000);
...
{
	const plainTransport = await ctx.router!.createPlainTransport(
		{ listenIp: '127.0.0.1', rtcpMux: false }
	);

	await expect(plainTransport.connect(
		{ ip: '1.2.3.4', port: 1234, rtcpPort: 1235 }
	))
		.resolves
		.toBeUndefined();
...
test('plainTransport.connect() with wrong arguments rejects with TypeError', async () =>
{
	...
	await expect(plainTransport.connect({ ip: '::::1234' }))
		.rejects
		.toThrow(TypeError);
...
test('PlainTransport methods reject if closed', async () =>
{
	...
	await expect(plainTransport.dump())
		.rejects
		.toThrow(Error);

	await expect(plainTransport.getStats())
		.rejects
		.toThrow(Error);
