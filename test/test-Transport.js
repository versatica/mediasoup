	...
	{ timeout: 2000 }, (t) =>
	{
		return initTest(t)
			.then((data) =>
			{
				...
				t.equal(
					transport.dtlsLocalParameters.role, 'auto',
				...
				return transport.setRemoteDtlsParameters(
					...
					.then(() =>
					{
						...
						t.equal(
							transport.dtlsLocalParameters.role, 'client',
					...
					.then((data2) =>
					{
						...
						t.equal(
							data2.dtlsLocalParameters.role, 'client',
...
tap.test(
	'transport.close() must succeed', { timeout: 2000 }, (t) =>
	{
		const server = mediasoup.Server();
						...
						transport.on('close', (error) =>
						{
							...
							peer.dump()
								.then((data) =>
								{
									...
									t.equal(
										Object.keys(data.transports).length, 0,
										'peer.dump() must retrieve zero transports');
								})
								.catch((error2) => t.fail(`peer.dump() failed: ${error2}`));
						});
...
tap.test(
	'create transport in a server with rtcAnnouncedIPv4', { timeout: 2000 }, (t) =>
	{
		const server = mediasoup.Server(
			{
				rtcIPv4          : true,
		...
		return server.createRoom(roomOptions)
			...
			.then((transport) =>
			{
				const candidates = transport.iceLocalCandidates;
