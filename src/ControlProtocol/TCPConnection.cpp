#define MS_CLASS "ControlProtocol::TCPConnection"
...
namespace ControlProtocol
{
	...
	void TCPConnection::userOnTCPConnectionRead(const MS_BYTE* data, size_t len)
	{
		...
		while (true)
		{
			...
			{
				// Notify the reader.
				this->reader->onControlProtocolMessage(this, msg, this->buffer + this->msgStart, parsed_len);
			...
			// Parsing error. Close the connection.
			else if (this->parser->HasError())
			{
				MS_ERROR("parsing error, closing the connection");
			...
			else
			{
				...
				if (this->bufferDataLen == this->bufferSize)
				{
					...
					// The message is too big, so close the connection.
					else
					{
						MS_ERROR("no more space in the buffer for the unfinished message being parsed, closing the connection");
