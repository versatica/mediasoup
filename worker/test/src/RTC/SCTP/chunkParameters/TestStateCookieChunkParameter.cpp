...
SCENARIO("State Cookie Chunk Parameter (7)", "[sctp][serializable]")
{
	...
	SECTION("StateCookieChunkParameter::Parse() succeeds")
	{
		// clang-format off
		uint8_t buffer[] =
		{
		...
		CHECK_PARAMETER(
		  ...
		  /*length*/ 8,
		  /*frozen*/ true,
		  /*parameterType*/ ChunkParameter::ChunkParameterType::STATE_COOKIE,
		  /*unknownType*/ false,
	...
	SECTION("StateCookieChunkParameter::Factory() succeeds")
	{
		auto* parameter = StateCookieChunkParameter::Factory(FactoryBuffer, sizeof(FactoryBuffer));
		...
		auto* parsedParameter =
		  StateCookieChunkParameter::Parse(parameter->GetBuffer(), parameter->GetLength());
