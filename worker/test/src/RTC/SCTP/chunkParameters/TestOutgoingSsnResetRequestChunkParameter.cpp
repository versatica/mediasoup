...
SCENARIO("Outgoing SSN Reset Request Chunk Parameter (13)", "[sctp][serializable]")
{
	resetBuffers();

	SECTION("OutgoingSsnResetRequestChunkParameter::Parse() succeeds")
	{
		// clang-format off
		uint8_t buffer[] =
		{
		...
		auto* parameter = OutgoingSsnResetRequestChunkParameter::Parse(buffer, sizeof(buffer));
		...
		CHECK_PARAMETER(
		  ...
		  /*length*/ 24,
		  /*frozen*/ true,
		  /*parameterType*/ ChunkParameter::ChunkParameterType::OUTGOING_SSN_RESET_REQUEST,
		  /*unknownType*/ false,
	...
	SECTION("OutgoingSsnResetRequestChunkParameter::Factory() succeeds")
	{
		auto* parameter =
		  OutgoingSsnResetRequestChunkParameter::Factory(FactoryBuffer, sizeof(FactoryBuffer));
		...
		auto* parsedParameter =
		  OutgoingSsnResetRequestChunkParameter::Parse(parameter->GetBuffer(), parameter->GetLength());
