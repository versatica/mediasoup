...
SCENARIO("Unrecognized Parameter Chunk Parameter (7)", "[sctp][serializable]")
{
	resetBuffers();

	SECTION("UnrecognizedParameterChunkParameter::Parse() succeeds")
	{
		// clang-format off
		uint8_t buffer[] =
		{
		...
		CHECK_PARAMETER(
		  ...
		  /*length*/ 8,
		  /*frozen*/ true,
		  /*parameterType*/ ChunkParameter::ChunkParameterType::UNRECOGNIZED_PARAMETER,
		  /*unknownType*/ false,
	...
	SECTION("UnrecognizedParameterChunkParameter::Factory() succeeds")
	{
		auto* parameter =
		  UnrecognizedParameterChunkParameter::Factory(FactoryBuffer, sizeof(FactoryBuffer));
		...
		auto* parsedParameter =
		  UnrecognizedParameterChunkParameter::Parse(parameter->GetBuffer(), parameter->GetLength());
