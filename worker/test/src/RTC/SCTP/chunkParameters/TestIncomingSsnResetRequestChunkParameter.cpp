...
SCENARIO("Incoming SSN Reset Request Chunk Parameter (14)", "[sctp][serializable]")
{
	resetBuffers();

	SECTION("IncomingSsnResetRequestChunkParameter::Parse() succeeds")
	{
		// clang-format off
		uint8_t buffer[] =
		{
		...
		auto* parameter = IncomingSsnResetRequestChunkParameter::Parse(buffer, sizeof(buffer));
		...
		CHECK_PARAMETER(
		  ...
		  /*length*/ 16,
		  /*frozen*/ true,
		  /*parameterType*/ ChunkParameter::ChunkParameterType::INCOMING_SSN_RESET_REQUEST,
		  /*unknownType*/ false,
	...
	SECTION("IncomingSsnResetRequestChunkParameter::Factory() succeeds")
	{
		auto* parameter =
		  IncomingSsnResetRequestChunkParameter::Factory(FactoryBuffer, sizeof(FactoryBuffer));
		...
		auto* parsedParameter =
		  IncomingSsnResetRequestChunkParameter::Parse(parameter->GetBuffer(), parameter->GetLength());
