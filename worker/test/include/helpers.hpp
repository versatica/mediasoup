...
namespace helpers
{
	...
	inline bool addToBuffer(uint8_t* buf, int* size, uint8_t* data, size_t len)
	{
	    ...
	    {
	        printf("Buffer index out of bound: size: %d index:%d\n",BUFFER_SIZE, *size);
	        return false;
	    }
	...
	{
	    std::string filePath = "test/data/naluInfo/" + std::string(file);
	#ifdef _WIN32
	    std::replace(filePath.begin(), filePath.end(), '/', '\\');
	#endif
	    std::ifstream in(filePath, std::ios::ate | std::ios::binary);
	...
	inline bool writeRtpPacket(const char* file, uint8_t nalType, size_t nalLength, int32_t sid, int32_t tid,
			int32_t isIdr, int32_t firstSliceId, int32_t lastSliceId, uint8_t* payload, uint8_t* buf, size_t* len)
	{
		std::string filePath = "test/data/naluInfo/" + std::string(file);
		#ifdef _WIN32
			std::replace(filePath.begin(), filePath.end(), '/', '\\');
	    ...
	    uint8_t buffer[16] = {
	                    ...
	                    98,245,71,218,
	                    159,113,8,226,
	                    190,222,0,1}; // extension header: extn header id, extn header length
	    ...
	    *len = packet_size;
	    // write the RTP packet to a binary file
//	    std::ofstream ofp(filePath, std::ios::out | std::ios::binary);
//	    ofp.write(reinterpret_cast<const char*>(buffer), packet_size);
//	    ofp.close();
