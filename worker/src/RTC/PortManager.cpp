...
namespace RTC
{
	...
	uv_handle_t* PortManager::Bind(Transport transport, std::string& ip)
	{
		...
		while (true)
		{
			...
			{
				MS_DEBUG_DEV(
					"port in use, trying again [%s:%s, attempt:%zu]",
				 	transportStr.c_str(), ip.c_str(), attempt);

				// If we have tried all ports in the vector throw.
				if (portIdx == initialPortIdx)
				{
					MS_THROW_ERROR(
						"no available port [%s:%s, attempt:%zu]",
						transportStr.c_str(), ip.c_str(), attempt);
				}
			...
			switch (transport)
			{
				case Transport::UDP:
					err = uv_udp_bind(
						reinterpret_cast<uv_udp_t*>(uvHandle), reinterpret_cast<const struct sockaddr*>(&bindAddr), flags);
					break;

				case Transport::TCP:
					err = uv_tcp_bind(
						reinterpret_cast<uv_tcp_t*>(uvHandle), reinterpret_cast<const struct sockaddr*>(&bindAddr), flags);
					break;
			}
			...
			MS_WARN_DEV(
				"uv_xxx_bind() failed [%s:%s, attempt:%zu]: %s",
			 	transportStr.c_str(), ip.c_str(), attempt, uv_strerror(err));
			...
			{
				MS_THROW_ERROR(
					"no more available ports [%s:%s, attempt:%zu]",
				 	transportStr.c_str(), ip.c_str(), attempt);
			}
			// Otherwise try again.
			else
