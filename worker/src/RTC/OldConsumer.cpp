#define MS_CLASS "RTC::OldConsumer"
...
namespace RTC
{
	...
	flatbuffers::Offset<FBS::Consumer::BaseConsumerDump> OldConsumer::FillBuffer(
	...
	void OldConsumer::HandleRequest(Channel::ChannelRequest* request)
	{
		MS_TRACE();
	...
	void OldConsumer::TransportConnected()
	{
		MS_TRACE();
	...
	void OldConsumer::TransportDisconnected()
	{
		MS_TRACE();
	...
	void OldConsumer::ProducerPaused()
	{
		MS_TRACE();
		...
		this->shared->GetChannelNotifier()->Emit(this->id, FBS::Notification::Event::CONSUMER_PRODUCER_PAUSE);
	}

	void OldConsumer::ProducerResumed()
	{
		MS_TRACE();
		...
		this->shared->GetChannelNotifier()->Emit(this->id, FBS::Notification::Event::CONSUMER_PRODUCER_RESUME);
	}

	void OldConsumer::ProducerRtpStreamScores(const std::vector<uint8_t>* scores)
	{
		MS_TRACE();
	...
	void OldConsumer::ProducerClosed()
	{
		MS_TRACE();
		...
		this->shared->GetChannelNotifier()->Emit(this->id, FBS::Notification::Event::CONSUMER_PRODUCER_CLOSE);

		this->listener->OnConsumerProducerClosed(this);
	}

	void OldConsumer::EmitTraceEventRtpAndKeyFrameTypes(const RTC::RTP::Packet* packet, bool isRtx) const
	{
		MS_TRACE();
	...
	void OldConsumer::EmitTraceEventPliType(uint32_t ssrc) const
	{
		MS_TRACE();
		...
		auto traceInfo =
		  FBS::Consumer::CreatePliTraceInfo(this->shared->GetChannelNotifier()->GetBufferBuilder(), ssrc);
	...
	void OldConsumer::EmitTraceEventFirType(uint32_t ssrc) const
	{
		MS_TRACE();
		...
		auto traceInfo =
		  FBS::Consumer::CreateFirTraceInfo(this->shared->GetChannelNotifier()->GetBufferBuilder(), ssrc);
	...
	void OldConsumer::EmitTraceEventNackType() const
	{
		MS_TRACE();
	...
	void OldConsumer::EmitTraceEvent(flatbuffers::Offset<FBS::Consumer::TraceNotification>& notification) const
	{
		MS_TRACE();
