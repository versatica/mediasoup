...
namespace RTC
{
	...
	std::map<std::string, struct sockaddr_storage> TcpServer::sockaddrStorageMultiIPv4s;
	std::map<std::string, struct sockaddr_storage> TcpServer::sockaddrStorageMultiIPv6s;
	...
	std::map<std::string, std::unordered_map<uint16_t, bool>> TcpServer::availableMultiIPv4sPorts;
	std::map<std::string, std::unordered_map<uint16_t, bool>> TcpServer::availableMultiIPv6sPorts;
	...
	void TcpServer::ClassInit()
	{
		...
		if (Settings::configuration.rtcMultiIPv4s.size() > 0)
		{
			     ...
			     it++)
			{
				...
				RTC::TcpServer::sockaddrStorageMultiIPv4s[it->first] = temp;
				...
				RTC::TcpServer::availableMultiIPv4sPorts[it->first] = tempMap;
			}
		}
		...
		if (Settings::configuration.rtcMultiIPv6s.size() > 0)
		{
			     ...
			     it++)
			{
				...
				RTC::TcpServer::sockaddrStorageMultiIPv6s[it->first] = temp;
				...
				RTC::TcpServer::availableMultiIPv6sPorts[it->first] = tempMap;
			}
		}
	...
	uv_tcp_t* TcpServer::GetRandomPort(int addressFamily, const std::string& ip)
	{
		MS_TRACE();
		...
		{
			case AF_INET:
				availablePorts = &RTC::TcpServer::availableMultiIPv4sPorts[ip];
				bindAddr       = RTC::TcpServer::sockaddrStorageMultiIPv4s[ip];
				listenIp       = ip.c_str();
				break;

			case AF_INET6:
				availablePorts = &RTC::TcpServer::availableMultiIPv6sPorts[ip];
				bindAddr       = RTC::TcpServer::sockaddrStorageMultiIPv6s[ip];
				listenIp       = ip.c_str();
				// Don't also bind into IPv4 when listening in IPv6.
				flags |= UV_TCP_IPV6ONLY;
				break;
	...
	TcpServer::TcpServer(
	  ...
	  : // Provide the parent class constructor with a UDP uv handle.
	    // NOTE: This may throw a MediaSoupError exception if the address family is not available
	    // or there are no available ports.
	    ::TcpServer::TcpServer(TcpServer::GetRandomPort(addressFamily, ip), 256), listener(listener),
	    connListener(connListener)
	{
		MS_TRACE();
	...
	void TcpServer::UserOnTcpServerClosed()
	{
		...
		else
		{
			...
			{
				auto availablePorts                = &RTC::TcpServer::availableMultiIPv4sPorts[ip];
				(*availablePorts)[this->localPort] = true;
			}

			else if (this->addressFamily == AF_INET6)
			{
				auto availablePorts                = &RTC::TcpServer::availableMultiIPv6sPorts[ip];
				(*availablePorts)[this->localPort] = true;
			}
		}
	}
