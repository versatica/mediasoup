...
namespace RTC
{
	...
	MultiStreamConsumer::MultiStreamConsumer(
	  RTC::Shared* shared,
	  const std::string& id,
	  ...
	  : RTC::Consumer::Consumer(shared, id, producerId, listener, data, type)
	{
		MS_TRACE();
		...
		MS_ASSERT(
		  ...
		  "type must be SIMPLE or SIMULCAST");
		...
		if (
		  this->encodingContext && mediaCodec->mimeType.type == RTC::RtpCodecMimeType::Type::AUDIO &&
	...
	flatbuffers::Offset<FBS::Consumer::DumpResponse> MultiStreamConsumer::FillBuffer(
	  flatbuffers::FlatBufferBuilder& builder) const
	{
		...
		auto dump = FBS::Consumer::CreateConsumerDumpDirect(
		  ...
		  this->encodingContext ? this->encodingContext->GetCurrentTemporalLayer() : 1);

		return FBS::Consumer::CreateDumpResponse(builder, dump);
	}

	flatbuffers::Offset<FBS::Consumer::GetStatsResponse> MultiStreamConsumer::FillBufferStats(
	...
	flatbuffers::Offset<FBS::Consumer::ConsumerScore> MultiStreamConsumer::FillBufferScore(
	  flatbuffers::FlatBufferBuilder& builder) const
	{
		MS_TRACE();
	...
	void MultiStreamConsumer::HandleRequest(Channel::ChannelRequest* request)
	{
		MS_TRACE();
	...
	void MultiStreamConsumer::ProducerRtpStream(RTC::RTP::RtpStreamRecv* rtpStream, uint32_t mappedSsrc)
	{
		MS_TRACE();
	...
	void MultiStreamConsumer::ProducerRtpStreamScore(
	  RTC::RTP::RtpStreamRecv* /*rtpStream*/, uint8_t score, uint8_t previousScore)
	{
		...
		if (RTC::Consumer::IsActive())
		{
			...
			if (!IsActive())
			{
				...
				{
					MS_DUMP("----- all Producers are inactive, calling UpdateTargetLayers(-1, -1)");
				}
	...
	void MultiStreamConsumer::ProducerRtcpSenderReport(RTC::RTP::RtpStreamRecv* rtpStream, bool first)
	{
		MS_TRACE();
	...
	uint32_t MultiStreamConsumer::IncreaseLayer(uint32_t bitrate, bool considerLoss)
	{
		MS_TRACE();
	...
	void MultiStreamConsumer::ApplyLayers()
	{
		MS_TRACE();
		...
		if (provisionalTargetLayers != this->targetLayers)
		{
			...
			{
				MS_DUMP(
				  "----- calling UpdateTargetLayers(%d, %d)",
				  provisionalTargetLayers.spatial,
				  provisionalTargetLayers.temporal);
			}
	...
	uint32_t MultiStreamConsumer::GetDesiredBitrate() const
	{
		MS_TRACE();
	...
	void MultiStreamConsumer::SendRtpPacket(RTC::RTP::Packet* packet, RTC::RTP::SharedPacket& sharedPacket)
	{
		...
		if (!IsActive())
		{
			...
			if (spatialLayer == this->currentSpatialLayer)
			{
				...
				{
					MS_DUMP(
					  "----- audio packet dropped CONSUMER_INACTIVE | seq:%u", packet->GetSequenceNumber());
				}
			...
			// TODO: REMOVE
			if (this->kind == RTC::Media::Kind::AUDIO && spatialLayer != this->currentSpatialLayer)
			{
				MS_DUMP(
				  "----- !IsActive() => audio packet ignored because spatialLayer %d != this->currentSpatialLayer %d | seq:%u",
				  spatialLayer,
				  this->currentSpatialLayer,
		...
		if (this->targetLayers.temporal == -1)
		{
			...
			if (spatialLayer == this->currentSpatialLayer)
			{
				...
				{
					MS_DUMP(
					  "----- audio packet dropped INVALID_TARGET_LAYER | seq:%u", packet->GetSequenceNumber());
				}
			...
			{
				MS_DUMP(
				  "----- this->targetLayers.temporal == -1 => audio packet ignored because spatialLayer %d != this->currentSpatialLayer %d | seq:%u",
				  spatialLayer,
				  this->currentSpatialLayer,
				  packet->GetSequenceNumber());
			}
		...
		if (!this->supportedCodecPayloadTypes[payloadType])
		{
			...
			if (spatialLayer == this->currentSpatialLayer)
			{
				...
				{
					MS_DUMP(
					  "----- audio packet dropped UNSUPPORTED_PAYLOAD_TYPE | seq:%u",
					  packet->GetSequenceNumber());
				}
			...
			{
				MS_DUMP(
				  "----- unsupported codec => audio packet ignored because spatialLayer %d != this->currentSpatialLayer %d | seq:%u",
				  spatialLayer,
				  this->currentSpatialLayer,
				  packet->GetSequenceNumber());
			}
		  ...
		  spatialLayer == this->targetLayers.spatial)
		{
			...
			if (this->keyFrameSupported && !packet->IsKeyFrame())
			{
				...
				if (this->kind == RTC::Media::Kind::AUDIO)
				{
					MS_DUMP("----- audio packet dropped NOT_A_KEYFRAME | seq:%u", packet->GetSequenceNumber());
				}
		...
		else if (spatialLayer != this->currentSpatialLayer)
		{
			...
			{
				MS_DUMP(
				  "----- audio packet ignored (spatialLayer %d != this->currentSpatialLayer %d) | seq:%u",
				  spatialLayer,
				  this->currentSpatialLayer,
				  packet->GetSequenceNumber());
			}
		...
		if (packet->GetPayloadLength() == 0)
		{
			...
			if (spatialLayer == this->currentSpatialLayer)
			{
				...
				if (this->kind == RTC::Media::Kind::AUDIO)
				{
					MS_DUMP("----- audio packet dropped EMPTY_PAYLOAD | seq:%u", packet->GetSequenceNumber());
				}
		...
		if (isSyncPacket && (this->spatialLayerToSync == -1 || spatialLayer == this->spatialLayerToSync))
		{
			...
			if (shouldSwitchCurrentSpatialLayer && (packet->GetTimestamp() - tsOffset <= this->rtpStream->GetMaxPacketTs()))
			{
				...
				else if (tsExtraOffset > maxTsExtraOffset)
				{
					...
					{
						MS_DUMP(
						  "----- audio packet dropped TOO_HIGH_TIMESTAMP_EXTRA_NEEDED | seq:%u",
						  packet->GetSequenceNumber());
					}
		...
		if (!shouldSwitchCurrentSpatialLayer && this->checkingForOldPacketsInSpatialLayer)
		{
			      ...
			      packet->GetSequenceNumber(), this->snReferenceSpatialLayer))
			{
				...
				{
					MS_DUMP(
					  "----- audio packet dropped PACKET_PREVIOUS_TO_SPATIAL_LAYER_SWITCH | seq:%u",
					  packet->GetSequenceNumber());
				}
		...
		if (shouldSwitchCurrentSpatialLayer)
		{
			...
			{
				MS_DUMP(
				  "----- setting this->currentSpatialLayer = %d [previous value:%d] | seq:%u",
				  this->targetLayers.spatial,
				  this->currentSpatialLayer,
				  packet->GetSequenceNumber());
			}
		...
		else if (this->encodingContext)
		{
			...
			if (!packet->ProcessPayload(this->encodingContext.get(), marker))
			{
				...
				if (this->kind == RTC::Media::Kind::AUDIO)
				{
					MS_DUMP("----- audio packet dropped DROPPED_BY_CODEC | seq:%u", packet->GetSequenceNumber());
				}
		...
		else
		{
			...
			if (this->kind == RTC::Media::Kind::AUDIO)
			{
				MS_DUMP("----- audio packet dropped SEND_RTP_STREAM_DISCARDED | seq:%u", origSeq);
			}
		}
	...
	bool MultiStreamConsumer::GetRtcp(RTC::RTCP::CompoundPacket* packet, uint64_t nowMs)
	{
		MS_TRACE();
	...
	void MultiStreamConsumer::NeedWorstRemoteFractionLost(
	...
	void MultiStreamConsumer::ReceiveNack(RTC::RTCP::FeedbackRtpNackPacket* nackPacket)
	{
		MS_TRACE();
	...
	void MultiStreamConsumer::ReceiveKeyFrameRequest(
	...
	void MultiStreamConsumer::ReceiveRtcpReceiverReport(RTC::RTCP::ReceiverReport* report)
	{
		MS_TRACE();
	...
	void MultiStreamConsumer::ReceiveRtcpXrReceiverReferenceTime(RTC::RTCP::ReceiverReferenceTime* report)
	{
		MS_TRACE();
	...
	uint32_t MultiStreamConsumer::GetTransmissionRate(uint64_t nowMs)
	{
		MS_TRACE();
	...
	float MultiStreamConsumer::GetRtt() const
	{
		MS_TRACE();
	...
	void MultiStreamConsumer::UserOnTransportConnected()
	{
		MS_TRACE();
	...
	void MultiStreamConsumer::UserOnTransportDisconnected()
	{
		MS_TRACE();
		...
		{
			MS_DUMP(
			  "----- calling UpdateTargetLayers(-1, -1) [current this->currentSpatialLayer:%d]",
			  this->currentSpatialLayer);
		}
	...
	void MultiStreamConsumer::UserOnPaused()
	{
		MS_TRACE();
	...
	void MultiStreamConsumer::UserOnResumed()
	{
		MS_TRACE();
	...
	void MultiStreamConsumer::CreateRtpStream()
	{
		MS_TRACE();
	...
	void MultiStreamConsumer::RequestKeyFrames()
	{
		MS_TRACE();
	...
	void MultiStreamConsumer::RequestKeyFrameForTargetSpatialLayer()
	{
		MS_TRACE();
	...
	void MultiStreamConsumer::RequestKeyFrameForCurrentSpatialLayer()
	{
		MS_TRACE();
	...
	void MultiStreamConsumer::MayChangeLayers(bool force)
	{
		MS_TRACE();
	...
	bool MultiStreamConsumer::RecalculateTargetLayers(RTC::ConsumerTypes::VideoLayers& newTargetLayers) const
	{
		MS_TRACE();
	...
	void MultiStreamConsumer::UpdateTargetLayers(
	  int16_t newTargetSpatialLayer, int16_t newTargetTemporalLayer)
	{
		...
		if (newTargetSpatialLayer == -1)
		{
			...
			{
				MS_DUMP(
				  "----- newTargetSpatialLayer == -1 => setting this->targetLayers.spatial = -1, this->targetLayers.temporal = -1, this->currentSpatialLayer = -1");
			}
		...
		{
			MS_DUMP(
			  "----- setting this->targetLayers.spatial = %d, this->targetLayers.temporal = %d",
		...
		if (this->encodingContext && this->targetLayers.spatial == this->currentSpatialLayer)
		{
			this->encodingContext->SetTargetTemporalLayer(this->targetLayers.temporal);
		}
	...
	bool MultiStreamConsumer::CanSwitchToSpatialLayer(int16_t spatialLayer) const
	{
		MS_TRACE();
		...
		MS_ASSERT(
		  this->producerRtpStreams.size() > spatialLayer,
	...
	void MultiStreamConsumer::StorePacketInTargetLayerRetransmissionBuffer(
	...
	void MultiStreamConsumer::EmitScore() const
	{
		MS_TRACE();
	...
	void MultiStreamConsumer::EmitLayersChange() const
	{
		MS_TRACE();
	...
	RTC::RTP::RtpStreamRecv* MultiStreamConsumer::GetProducerTargetRtpStream() const
	{
		MS_TRACE();
	...
	RTC::RTP::RtpStreamRecv* MultiStreamConsumer::GetProducerTsReferenceRtpStream() const
	{
		MS_TRACE();
	...
	void MultiStreamConsumer::OnRtpStreamScore(
	...
	void MultiStreamConsumer::OnRtpStreamRetransmitRtpPacket(
