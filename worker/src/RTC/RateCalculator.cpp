...
namespace RTC
{
	...
	void RateCalculator::Update(size_t size, uint64_t nowMs)
	{
		...
		if (this->oldestItemStartTime.has_value() && TimeManager::IsTimeLowerThan(nowMs, *this->oldestItemStartTime))
		{
		...
		if (
		  ...
		  TimeManager::IsTimeHigherOrEqualThan(nowMs - *this->newestItemStartTime, this->itemSizeMs))
		{
	...
	void RateCalculator::RemoveOldData(uint64_t nowMs)
	{
		...
		if (TimeManager::IsTimeLowerThan(newOldestTime, *this->oldestItemStartTime))
		{
			return;
		}
		...
		if (
		  ...
		  TimeManager::IsTimeHigherOrEqualThan(newOldestTime, *this->newestItemStartTime))
		{
		...
		while (TimeManager::IsTimeHigherOrEqualThan(newOldestTime, *this->oldestItemStartTime))
		{
