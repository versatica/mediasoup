...
uint32_t ConvertMsTo24Bits(int64_t timeMs) {
  uint32_t time_24Bits =
      static_cast<uint32_t>(
          ((static_cast<uint64_t>(timeMs) << kAbsSendTimeFraction) + 500) /
          1000) &
      0x00FFFFFF;
  return time_24Bits;
}
...
RemoteBitrateEstimatorAbsSendTime::ProcessClusters(int64_t nowMs) {
  std::list<Cluster> clusters;
  ...
  if (bestIt != clusters.end()) {
    int probeBitrateBps =
        std::min(bestIt->GetSendBitrateBps(), bestIt->GetRecvBitrateBps());
...
void RemoteBitrateEstimatorAbsSendTime::IncomingPacket(
  ...
  IncomingPacketInfo(arrivalTimeMs, Utils::Byte::Get3Bytes(absoluteSendTime, 0),
                     payloadSize, packet.GetSsrc());
}
...
void RemoteBitrateEstimatorAbsSendTime::IncomingPacketInfo(
  ...
  std::vector<uint32_t> ssrcs;
  {
    ...
    if (this->interArrival->ComputeDeltas(timestamp, arrivalTimeMs, nowMs,
                                      payloadSize, &tsDelta, &tDelta,
                                      &sizeDelta)) {
      double tsDeltaMs = (1000.0 * tsDelta) / (1 << kInterArrivalShift);
      this->estimator->Update(tDelta, tsDeltaMs, sizeDelta, this->detector.State(),
                         arrivalTimeMs);
      this->detector.Detect(this->estimator->GetOffset(), tsDeltaMs,
                       this->estimator->GetNumOfDeltas(), arrivalTimeMs);
    }
