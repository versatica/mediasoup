...
namespace RTC
{
	  ...
	  : RTC::Transport::Transport(shared, id, listener, options->base())
	{
		...
		this->listenInfo.minPort            = options->listenInfo()->minPort();
		this->listenInfo.maxPort            = options->listenInfo()->maxPort();
		...
		if (!this->rtcpMux)
		{
			      ...
			      options, FBS::PlainTransport::PlainTransportOptions::VT_RTCPLISTENINFO))
			{
				...
				this->rtcpListenInfo.minPort        = options->rtcpListenInfo()->minPort();
				this->rtcpListenInfo.maxPort        = options->rtcpListenInfo()->maxPort();
		...
		try
		{
			if (this->listenInfo.minPort != 0 && this->listenInfo.maxPort != 0)
			{
				...
				this->udpSocket = new RTC::UdpSocket(
				  this,
				  this->listenInfo.ip,
				  this->listenInfo.minPort,
				  this->listenInfo.maxPort,
			...
			if (!this->rtcpMux)
			{
				if (this->rtcpListenInfo.minPort != 0 && this->rtcpListenInfo.maxPort != 0)
				{
					uint64_t portRangeHash{ 0u };
					...
					this->rtcpUdpSocket = new RTC::UdpSocket(
					  ...
					  this->rtcpListenInfo.minPort,
					  this->rtcpListenInfo.maxPort,
