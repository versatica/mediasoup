...
std::string ToString(const double d) {
	char buf[32];
	const int len = std::snprintf(&buf[0], ABSL_ARRAYSIZE(buf), "%g", d);
...
std::string ToString(const int s) {
	char buf[32];
	const int len = std::snprintf(&buf[0], ABSL_ARRAYSIZE(buf), "%d", s);
	// RTC_DCHECK_LE(len, arraysize(buf));
	return std::string(&buf[0], len);
}
...
std::string ToString(const unsigned int s) {
	char buf[32];
	const int len = std::snprintf(&buf[0], ABSL_ARRAYSIZE(buf), "%u", s);
	// RTC_DCHECK_LE(len, arraysize(buf));
	return std::string(&buf[0], len);
}
