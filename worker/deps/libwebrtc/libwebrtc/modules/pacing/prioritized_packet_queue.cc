...
DataSize PrioritizedPacketQueue::QueuedPacket::PacketSize() const {
  return DataSize::bytes(packet->GetPayloadLength() + packet->GetPayloadPadding());
}
...
void PrioritizedPacketQueue::Push(Timestamp enqueue_time,
  ...
  auto [it, inserted] = streams_.emplace(packet->GetSsrc(), nullptr);
  if (inserted) {
    it->second = absl::make_unique<StreamQueue>(enqueue_time);
  }
  ...
  RtpPacketMediaType packet_type = packet->GetPacketType();
...
std::unique_ptr<RTC::RtpPacket> PrioritizedPacketQueue::Pop() {
  if (size_packets_ == 0) {
    return nullptr;
  }
  ...
  RtpPacketMediaType packet_type = packet.packet->GetPacketType();
  --size_packets_per_media_type_[static_cast<size_t>(packet_type)];
  // RTC_DCHECK_GE(size_packets_per_media_type_[static_cast<size_t>(packet_type)], 0);
