...
#include "modules/pacing/rtp_packet_sender.h"
...
class PacingController {
 public:
  class PacketSender: RtpPacketSender {
   public:
    virtual ~PacketSender() = default;
    virtual void SendPacket(std::unique_ptr<RTC::RtpPacket> packet,
                            const PacedPacketInfo& cluster_info) = 0;
    // Should be called after each call to SendPacket().
    virtual std::vector<std::unique_ptr<RTC::RtpPacket>> FetchFec() = 0;
    virtual std::vector<std::unique_ptr<RTC::RtpPacket>> GeneratePadding(
        DataSize size) = 0;
    ...
    virtual void OnAbortedRetransmissions(
        uint32_t ssrc,
        std::vector<const uint16_t> sequence_numbers) {}
  ...
  // it's time to send.
  void EnqueuePacket(std::unique_ptr<RTC::RtpPacket> packet);
 ...
 private:
  ...
  std::unique_ptr<RTC::RtpPacket> GetPendingPacket(
