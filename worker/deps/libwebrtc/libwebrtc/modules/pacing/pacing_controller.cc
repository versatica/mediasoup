...
bool IsDisabled(const WebRtcKeyValueConfig& field_trials, absl::string_view key) {
  return absl::StartsWith(field_trials.Lookup(key), "Disabled");
}

bool IsEnabled(const WebRtcKeyValueConfig& field_trials, absl::string_view key) {
...
PacingController::PacingController(PacketSender* packet_sender,
    ...
    : packet_sender_(packet_sender),
      ...
      last_timestamp_(Timestamp::ms(DepLibUV::GetTimeMsInt64())),
      paused_(false),
      ...
      last_process_time_(Timestamp::ms(DepLibUV::GetTimeMsInt64())),
...
void PacingController::CreateProbeCluster(DataRate bitrate, int cluster_id) {
	...
	config.at_time = CurrentTime();
	config.target_data_rate = bitrate;
	config.target_duration = TimeDelta::ms(15);
	config.target_probe_count = 5;
	config.id = cluster_id;

  prober_.CreateProbeCluster(config);
}
...
Timestamp PacingController::CurrentTime() const {
  Timestamp time = Timestamp::ms(DepLibUV::GetTimeMsInt64());
...
void PacingController::SetPacingRates(DataRate pacing_rate,
  ...
  static constexpr DataRate kMaxRate = DataRate::KilobitsPerSec<100000>();
...
void PacingController::EnqueuePacket(std::unique_ptr<RTC::RtpPacket> packet) {
  // RTC_DCHECK(pacing_rate_ > DataRate::Zero()) << "SetPacingRate must be called before InsertPacket.";
  // RTC_CHECK(packet->GetPacketType());

  prober_.OnIncomingPacket(DataSize::bytes(packet->GetPayloadLength()));
...
void PacingController::ProcessPackets() {
  ...
  if (ShouldSendKeepalive(now)) {
    ...
    if (seen_first_packet_) {
      std::vector<std::unique_ptr<RTC::RtpPacket>> keepalive_packets =
          packet_sender_->GeneratePadding(DataSize::bytes(1));
      for (auto& packet : keepalive_packets) {
        keepalive_data_sent +=
            DataSize::bytes(packet->GetPayloadLength() + packet->GetPayloadPadding());
  ...
  for (; iteration < kMaxIterations; ++iteration) {
    ...
    if (rtp_packet == nullptr) {
      ...
      if (padding_to_add > DataSize::Zero()) {
        std::vector<std::unique_ptr<RTC::RtpPacket>> padding_packets =
      ...
      const RtpPacketMediaType packet_type = rtp_packet->GetPacketType();
      DataSize packet_size = DataSize::bytes(rtp_packet->GetPayloadLength() +
                                             rtp_packet->GetPayloadPadding());

      if (include_overhead_) {
        packet_size += DataSize::bytes(rtp_packet->GetSize() - rtp_packet->GetPayloadLength() - rtp_packet->GetPayloadPadding()) +
                       transport_overhead_per_packet_;
      }
...
std::unique_ptr<RTC::RtpPacket> PacingController::GetPendingPacket(
...
void PacingController::MaybeUpdateMediaRateDueToLongQueue(Timestamp now) {
  ...
  if (queue_size_data > DataSize::Zero()) {
    ...
    if (min_rate_needed > pacing_rate_) {
      ...
      // RTC_LOG(LS_VERBOSE) << "bwe:large_pacing_queue pacing_rate_kbps=" << pacing_rate_.kbps();
    }
  }
}
