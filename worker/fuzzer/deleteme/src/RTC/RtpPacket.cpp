...
namespace RTC
{
	...
	RtpPacket* RtpPacket::Parse(const uint8_t* data, size_t len)
	{
		...
		if (header->csrcCount != 0u)
		{
			...
			{
				MS_WARN_DEV("not enough space for the announced CSRC list, packet discarded");
		...
		if (header->extension != 0u)
		{
			...
			{
				MS_WARN_DEV("not enough space for the announced extension header, packet discarded");
			...
			{
				MS_WARN_DEV("not enough space for the announced header extension value, packet discarded");
		...
		if (header->padding != 0u)
		{
			...
			if (payloadLength == 0)
			{
				MS_WARN_DEV("padding bit is set but no space for a padding byte, packet discarded");
			...
			if (payloadPadding == 0)
			{
				MS_WARN_DEV("padding byte cannot be 0, packet discarded");
	...
	void RtpPacket::ParseExtensions()
	{
		...
		if (HasOneByteExtensions())
		{
			...
			while (ptr < extensionEnd)
			{
				...
				{
					MS_WARN_DEV("not enough space for the announced One-Byte header extension element value");
		...
		else if (HasTwoBytesExtensions())
		{
			...
			while (ptr + 1 < extensionEnd)
			{
				...
				{
					MS_WARN_DEV("not enough space for the announced Two-Bytes header extension element value");
