...
class RTCPeerConnectionPlanB extends RTCPeerConnectionCommon
{
	constructor(options)
	{
		...
		// Map of Producers indexed by MediaStreamTrack.id (a=label).
		this._mapTrackToProducer = new Map();
	}
	...
	_setUpOffer()
	{
		...
		return Promise.resolve()
			// First handle all the Consumers.
			.then(() =>
			{
				const promises = [];
				...
				for (const consumer of this._peer.consumers)
				{
					...
					const promise = consumer.setTransport(transport)
						.then(() =>
						{
							...
							consumer.on('activechange', (active) =>
							{
								this._logger.debug(
									'consumer "activechange" event [active:%s]', active);
			...
			.then(() =>
			{
				...
				for (const parsedMedia of parsed.media)
				{
					...
					for (const ssrc of ssrcs)
					{
						...
						const promise =
							this._createProducerForSsrc(parsedMedia, ssrc, trackInfo)
								.then((producer) =>
								{
									// Store it in the map.
									this._mapTrackToProducer.set(trackInfo.trackId, producer);

									// When closed, remove from the map.
									producer.on('close', () =>
									{
										this._mapTrackToProducer.delete(trackInfo.trackId);
									});
								});
			...
			.then(() =>
			{
				...
				for (const parsedMedia of parsed.media)
				{
					...
					for (const ssrc of ssrcs)
					{
						...
						if (!this._mapTrackToProducer.has(trackInfo.trackId))
						{
							this._logger.debug(
								'_handleRemoteReAnswer() | new receiving track ' +
								'[ssrc:%s, trackId:"%s"]',
				...
				for (const trackId of this._mapTrackToProducer.keys())
				{
					if (!receivingTrackIds.has(trackId))
					{
						this._logger.debug(
							'_handleRemoteReAnswer() | receiving track removed [trackId:"%s"]',
							trackId);

						const producer = this._mapTrackToProducer.get(trackId);
	...
	_createLocalDescription(type)
	{
		...
		function addMediaSection(kind)
		{
			...
			else
			{
				...
				for (const consumer of self._peer.consumers)
				{
					// Just the same kind.
						...
						objMedia.ssrcs.push(
							{
								id        : encoding.ssrc,
								attribute : 'msid',
								value     : consumer.rtpParameters.userParameters.msid
							});
				...
				// TODO: Just set this if enabled via API (and once simulcast is
				// implemented).
				// if (kind === 'video')
	...
	_createProducerForSsrc(parsedMedia, mediaSsrcs, trackInfo)
	{
