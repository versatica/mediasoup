...
class RTCPeerConnectionCommon extends EventEmitter
{
	constructor(options)
	{
		...
		this._logger = new Logger(`webrtc:RTCPeerConnection:${options.peer.name}`);
		...
		// Handle peer 'newconsumer' event once first SDP O/A is done.
		this._peer.on('newconsumer', () =>
		{
			this._logger.debug('peer "newconsumer" event');
	...
	setCapabilities(sdp)
	{
		...
		return Promise.resolve()
			...
			.then(() =>
			{
				const capabilities = sdpUtils.descToCapabilities(desc.parsed);
			...
			.then((transport) =>
			{
				// If the transport is abruptly closed (for example, a DTLS Close Alert is
				// received) close the peer.
	...
	createOffer(options)
	{
		...
		return this._setUpOffer()
			.then(() =>
			{
				...
				// Create an offer.
				const localDescription = this._createLocalDescription('offer');
	...
	setRemoteDescription(desc)
	{
		...
		catch (error)
		{
			return Promise.reject(
				new Error(`invalid RTCSessionDescriptionInit: ${error}`));
		}
		...
		switch (remoteDescription.type)
		{
			case 'offer':
			{
				...
				{
					return Promise.reject(
						new Error('initial offer from the endpoint not supported'));
				}
				// Re-offer received from the client.
				else
				{
					return Promise.reject(
						new Error('re-offer from the endpoint not supported'));
				}
			}
