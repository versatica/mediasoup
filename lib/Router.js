...
class Router extends EnhancedEventEmitter_1.EnhancedEventEmitter {
     ...
     */
    async createDataTransport({ enableSctp = true, maxSctpMessageSize = 262144, appData = {} }) {
        logger.debug('createDataTransport()');
        const internal = { ...this._internal, transportId: uuid_1.v4() };
        const reqData = { enableSctp, maxSctpMessageSize };
        const data = await this._channel.request('router.createDataTransport', internal, reqData);
        const transport = new DataTransport_1.DataTransport({
            internal,
            data,
            channel: this._channel,
            payloadChannel: this._payloadChannel,
