...
class Router extends EnhancedEventEmitter
{
	 ...
	 *   streams (requested outbound streams and maximum inbound streams).
   ...
   *
	 ...
	 *   announcedIp string.
	 * @param {Boolean} [enableSctp=false] - Enable SCTP.
	 * @param {Object} [numSctpStreams={ OS: 65535, MIS: 65535 }] - Number of SCTP
	 *   streams (requested outbound streams and maximum inbound streams).
		...
		else
		{
			try
			{
				pipeTransportPair = await Promise.all(
					[
						this.createPipeTransport({ listenIp, enableSctp: true }),
						router.createPipeTransport({ listenIp, enableSctp: true })
					]);
