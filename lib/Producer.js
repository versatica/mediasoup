...
class Producer extends EnhancedEventEmitter
{
	...
	close(appData, notifyChannel = true)
	{
		...
		if (!this.transport.closed)
			this.emit('@notify', 'producerClosed', { id: this.id, appData });
	...
	pause(appData)
	{
		...
		this.emit('@notify', 'producerPaused', { id: this.id, appData });
	...
	resume(appData)
	{
		...
		this.emit('@notify', 'producerResumed', { id: this.id, appData });
