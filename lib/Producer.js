...
class Producer extends EnhancedEventEmitter
{
			...
			switch (event)
			{
				case 'rtpraw':
				{
					...
					this._channel.request(
						...
						{
							logger.error(
								'"producer.setRtpRawEvent" request failed: %o', error);
						});
				...
				case 'rtpobject':
				{
					...
					this._channel.request(
						...
						{
							logger.error(
								'"producer.setRtpObjectEvent" request failed: %o', error);
						});
	...
	_destroy(notifyChannel = true)
	{
		...
		if (notifyChannel)
		{
			...
			this._channel.request('producer.close', this._internal)
				...
				.catch((error) =>
				{
					logger.error('"producer.close" request failed: %o', error);
				});
		}
	}
	...
	dump()
	{
		...
		return this._channel.request('producer.dump', this._internal)
			...
			.catch((error) =>
			{
				logger.error('"producer.dump" request failed: %o', error);
	...
	receive(rtpParameters, rtpMapping)
	{
		...
		return this._channel.request('producer.receive', this._internal, data)
			...
			.catch((error) =>
			{
				logger.error('"producer.receive" request failed: %o', error);
