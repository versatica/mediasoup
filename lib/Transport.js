...
class Transport extends EventEmitter
{
		...
		this._channel.on(this._internal.transportId, (event, data) =>
		{
			switch (event)
			{
				...
				case 'icestatechange':
				{
					...
					// Unset .iceSelectedTuple if required
					if (data.iceState === 'disconnected')
					{
	...
	close(error, dontSendChannel)
	{
		...
		// We won't receive events anymore, so be nice and manually update
		// some fields and emit events before firing 'close'

		delete this._data.iceSelectedTuple;

		if (this._data.iceState !== 'closed')
		{
			this._data.iceState = 'closed';
			this.emit('icestatechange', { iceState: 'closed' });
		}

		if (!/^failed|closed$/.test(this._data.dtlsState))
		{
			this._data.dtlsState = 'closed';
			this.emit('dtlsstatechange', { dtlsState: 'closed' });
		}
