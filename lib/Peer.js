	...
	_createProducer(id, kind, transportId, appData)
	{
		...
		return this._channel.request('router.createProducer', internal, options)
			.then(() =>
			{
				...
				// TODO: Do we need this? The C++ Transport closes all its Producers and
				// Consumers, and the client side gets producer.on('unhandled'), there is
				// no need to close/destroy its local Producer.
