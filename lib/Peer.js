...
class Peer extends EnhancedEventEmitter
{
	...
	handleNewPeer(peer)
	{
		const consumers = this.consumers
			.filter((consumer) => consumer.peer === peer);

		const peerData =
			Object.assign({ consumers }, peer.toJson());

		this.emit('@notify', 'newPeer', peerData);
	}

	/**
	 * @private
	 */
	createConsumerForProducer(producer)
	{
		...
		this.emit('@notify', 'newConsumer', consumer.toJson());
	}
