...
class Peer extends EnhancedEventEmitter
{
			...
			.then(() =>
			{
				switch (method)
				{
					case 'createTransport':
					{
						...
						return this._createTransport(id, direction, options, appData)
							.then((transport) =>
							{
					...
					case 'enableConsumer':
					{
						...
						return consumer.enable(transport, paused)
							.then(() =>
							{
								return {
	...
	_createTransport(id, direction, options, appData)
	{
		logger.debug('_createTransport() [id:%s, direction:%s]', id, direction);
