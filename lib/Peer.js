	...
	_createProducer(id, kind, transportId, appData)
	{
		...
		return this._channel.request('router.createProducer', internal, options)
			.then(() =>
			{
				...
				// On closure, the worker Transport closes all its Producers and Consumers,
				// and the client side gets producer.on('unhandled'), so there is no need
				// to close/destroy its local Producer.
				transport.on('@close', () =>
				{
					if (!producer.closed)
