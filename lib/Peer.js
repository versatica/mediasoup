...
class Peer extends EventEmitter
{
	...
	_createTransport({ id, options, remoteDtlsParameters })
	{
		...
		return this._channel.request('peer.createTransport', internal, options)
			...
			.then(() =>
			{
				// TODO: This must be properly done in the worker.
				transport.iceLocalParameters.iceLite = true;
				transport.dtlsLocalParameters.fingerprints =
					Object.keys(transport.dtlsLocalParameters.fingerprints)
						.map((hash) =>
						{
							return {
								algorithm : hash,
								value     : transport.dtlsLocalParameters.fingerprints[hash]
							};
						});
