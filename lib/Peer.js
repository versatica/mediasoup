...
{
	constructor(internal, channel, extra)
	{
		...
		this._channel.on(this._internal.peerId, (event, data) =>
		{
			switch (event)
			{
				...
				case 'newrtpsender':
				{
					let receiverPeer = this._extra.getPeer(data.peerName);

					if (!receiverPeer)
					{
						logger.error('"newrtpsender" event for a non existing receiver Peer [peerName:"%s"]', data.peerName);
	...
	/**
	 * Get an array with all the RtcSenders
	 * @return {Array<RtcSenders>}
	 */
	getRtcSenders()
	{
