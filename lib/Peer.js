...
class Peer extends EnhancedEventEmitter
{
			...
			.then(() =>
			{
				switch (method)
				{
					case 'createTransport':
					{
						...
						return this._createTransport(id, options, appData)
							.then((transport) =>
							{
					...
					case 'createProducer':
					{
						...
						return this._createProducer(id, kind, transportId, rtpParameters, appData)
							.then((producer) =>
							{
								if (paused)
					...
					{
						// TODO: Use paused!
						const { id, transportId, paused } = request;
						...
						// TODO: Get the RtpParameters of the associated Producer and "merge"
						// them with the Peer's rtpCapabilities to produce sending rtpParameters.
						const rtpParameters = {};

						return consumer.enable(transport, rtpParameters);
					}
	...
	{
		// TODO: Check if producer is paused.
		// TODO: So Consumer.js must have sourcePause() and sourceResume() methods.
	...
	_createTransport(id, options, appData)
	{
		...
		return this._channel.request('router.createTransport', internal, options)
			.then((data) =>
			{
				logger.debug('"router.createTransport" request succeeded');
			...
			.catch((error) =>
			{
				logger.error('"router.createTransport" request failed: %s', String(error));

				throw error;
			});
	}

	_createProducer(id, kind, transportId, rtpParameters, appData)
	{
		...
		try
		{
			rtpMapping = this._sandbox.getProducerRtpParametersMapping(rtpParameters);
		}
		catch (error)
		{
