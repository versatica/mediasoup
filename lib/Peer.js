...
class Peer extends EnhancedEventEmitter
{
	...
	_destroy(notifyChannel = true)
	{
		...
		if (notifyChannel)
		{
			...
			this._channel.request('peer.close', this._internal)
				...
				.catch((error) =>
				{
					logger.error('"peer.close" request failed: %o', error);
				});
		}
	}
	...
	dump()
	{
		...
		return this._channel.request('peer.dump', this._internal)
			...
			.catch((error) =>
			{
				logger.error('"peer.dump" request failed: %o', error);
	...
	{
		logger.debug('setRtpCapabilities()');
			...
			.then(() =>
			{
				switch (method)
				{
					case 'createTransport':
					{
						...
						return this._createTransport(id, options)
							...
							.then(() =>
							{
								const response =
								{
									iceParameters  : newTransport.iceLocalParameters,
									iceCandidates  : newTransport.iceLocalCandidates,
									dtlsParameters : newTransport.dtlsLocalParameters
								};
					...
					case 'createProducer':
					{
						...
						// NOTE: This may throw.
						const { newRtpParameters, rtpMapping } =
							this._sandbox.adaptProducerRtpParametersToRtpCapabilities(
								rtpParameters);

						// console.log('------ createProducer: original rtpParameters:');
						// console.log(JSON.stringify(rtpParameters, null, '  '));
						// console.log('------ createProducer: new rtpParameters:');
						// console.log(JSON.stringify(newRtpParameters, null, '  '));
						// console.log('------ rtp mapping::');
						// console.log(JSON.stringify(rtpMapping, null, '  '));
						...
						return this._createProducer(id, kind, transportId)
							.then((producer) =>
							{
								...
								return producer.receive(newRtpParameters, rtpMapping)
									.catch((error) =>
									{
										producer.close();
	...
	_createTransport(id, options)
	{
		...
		return this._channel.request('peer.createTransport', internal, options)
			...
			.catch((error) =>
			{
				logger.error('"peer.createTransport" request failed: %o', error);
	...
	_createProducer(id, kind, transportId)
	{
		...
		return this._channel.request('peer.createProducer', internal, options)
			.then(() =>
			{
				...
				transport.on('close', () => producer.close());
			...
			.catch((error) =>
			{
				logger.error('"peer.createProducer" request failed: %o', error);
