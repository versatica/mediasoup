...
class Peer extends EnhancedEventEmitter
{
	...
	Producer(kind, transport)
	{
		...
		else if (!this._transports.has(transport.id))
		...
		return Promise.resolve()
			.then(() =>
			{
				switch (method)
				{
					...
					{
						const { id, options, dtlsParameters } = request;
						...
						return this._createTransport(id, options)
							...
							.then((transport) =>
							{
								transport.appData = request.appData;
					...
					case 'createProducer':
					{
						...
						// TODO: create Producer and call send().
