...
class RTCPeerConnectionUnifiedPlan extends RTCPeerConnectionCommon
{
	...
	_handleRemoteReOffer(remoteDescription) // eslint-disable-line no-unused-vars
	{
		...
		function updateTransceiver(transceiver, parsedMedia)
		{
			...
			if (transceiver.receiver || transceiver.direction === 'recvonly')
			{
				// Peer has added a sending track on a slot reserved for it.
				if (
					(!transceiver.receiver || transceiver.receiver.closed) &&
					(parsedMedia.direction === 'sendonly' || parsedMedia.direction === 'sendrecv'))
				{
					self._logger.debug('_handleRemoteReOffer() | new receiving transceiver [mid:%s]', mid);
				...
				// Peer has closed a sending track.
					...
					(parsedMedia.direction === 'inactive' || parsedMedia.direction === 'recvonly'))
				{
					self._logger.debug('_handleRemoteReOffer() | receiving transceiver closed [mid:%s]', mid);
