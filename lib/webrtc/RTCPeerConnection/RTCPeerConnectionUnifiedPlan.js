	...
	_setUpOffer()
	{
		...
		return Promise.resolve()
			...
			.then(() =>
			{
				...
				{
					const mid = rtpSender.rtpParameters.muxId;
					...
					const promise = rtpSender.setTransport(transport)
						...
						.then(() =>
						{
							const transceiver = new RTCRtpTransceiver(mid, rtpSender.kind);
		...
		return Promise.resolve()
			...
			.then(() =>
			{
				...
				for (const parsedMedia of parsed.media)
				{
					...
					const promise = this._createRtpReceiver(parsedMedia)
						.then((rtpReceiver) =>
						{
		...
		function updateTransceiver(transceiver, parsedMedia)
		{
			...
			if (transceiver.direction === 'recvonly')
			{
				...
				if (!transceiver.receiver && parsedMedia.direction === 'sendonly')
				{
					...
					return self._createRtpReceiver(parsedMedia)
						.then((rtpReceiver) =>
						{
							...
							transceiver2.receiver = rtpReceiver;
						});
				}
					...
					(parsedMedia.direction === 'inactive' || parsedMedia.port === 0))
				{
					...
					const newTransceiver = new RTCRtpTransceiver(transceiver.mid, transceiver.kind);
