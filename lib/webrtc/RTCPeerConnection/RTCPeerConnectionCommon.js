...
class RTCPeerConnectionCommon extends EventEmitter
{
	constructor(options)
	{
		...
		// Flag indicating that ICE restart was performed.
	...
	setCapabilities(sdp)
	{
		...
		if (this.closed)
			return Promise.reject(InvalidStateError('closed'));

		if (this._peer.capabilities)
			return Promise.reject(InvalidStateError('capabilities are ready set'));
		...
		switch (remoteDescription.type)
		{
			...
			case 'answer':
			{
				...
				else
				{
					...
					return Promise.resolve()
						// Check remote DTLS parameters if ICE was restarted.
						.then(() =>
						{
						...
						// Run specific code for Plan-B and Unified-Plan.
						.then(() =>
						{
	...
	restartIce()
	{
		...
		let oldTransport = this._peer.transports[0];
		let newTransport;
		let promises = [];
		...
		return Promise.resolve()
			...
			.then((transport) =>
			{
				...
				// If maxBitrate was given, set it.
				if (this._options.maxBitrate)
					return newTransport.setMaxBitrate(this._options.maxBitrate);
			})
			// Assign it to all the RtpReceivers.
			.then(() =>
			{
				for (let rtpReceiver of this._peer.rtpReceivers)
				{
					if (rtpReceiver.closed)
						continue;

					let promise = rtpReceiver.setTransport(newTransport)
						.catch((error) =>
						{
							// It may happen that, at this point, the RtpReceivers was internally
							// closed, so don't abort everythin due to this.
							this._logger.error(`rtpReceiver.setTransport() failed: ${error}`);
						});

					promises.push(promise);
				}
			})
			// Assign it to all the RtpSenders.
			.then(() =>
			{
				for (let rtpSender of this._peer.rtpSenders)
				{
					...
					let promise = rtpSender.setTransport(newTransport)
						.catch((error) =>
						{
							// It may happen that, at this point, the RtpSenders was internally
							// closed, so don't abort everythin due to this.
			...
			.catch((error) =>
			{
				this._logger.error('_restartIce() | failed: %s', error);
