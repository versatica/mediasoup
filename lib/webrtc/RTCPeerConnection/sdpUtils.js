...
module.exports =
{
			...
			for (let group of parsedMedia.ssrcGroups)
			{
				switch (group.semantics)
				{
					case 'SIM':
					{
						...
						for (let ssrc of group.ssrcs.split(' '))
						{
							ssrc = Number(ssrc);
	...
	fixAnswerForIceRestart(parsedOffer, parsedAnswer)
	{
		for (let i=0, len=parsedOffer.media.length; i<len; ++i)
		{
			let parsedOfferMedia = parsedOffer.media[i];
			let parsedAnswerMedia = parsedAnswer.media[i];

			// Reset header extensions in the answer.
			delete parsedAnswerMedia.ext;

			if (!Array.isArray(parsedOfferMedia.ext))
				continue;

			// Clone extensions from the offer to the answer.
			parsedAnswerMedia.ext = utils.clone(parsedOfferMedia.ext);

			// Reverse direction.
			for (let ext of parsedAnswerMedia.ext)
			{
				if (ext.direction === 'sendonly')
					ext.direction = 'recvonly';
				else if (ext.direction === 'recvonly')
					ext.direction = 'sendonly';
			}
		}
	}
};
