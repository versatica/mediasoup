...
class RTCPeerConnectionPlanB extends RTCPeerConnectionCommon
{
	constructor(options)
	{
		...
		// Map of RtpReceivers indexed by SSRC of single streams (no simulcast).
		this._mapSingleSsrcToRtpReceivers = new Map();

		// TODO: Map of RtpReceivers indexed by simulcast msid
	}
		...
		return Promise.resolve()
			...
			.then(() =>
			{
				...
				for (let parsedMedia of parsed.media)
				{
					...
					for (let ssrc of ssrcs.singles)
					{
						let promise = this._createRtpReceiverForSsrc(ssrc, parsedMedia);

						promises.push(promise);
					}

					for (let simulcast of ssrcs.simulcasts)
					{
						let promise = this._createRtpReceiverForSsrc(simulcast, parsedMedia);
			...
			.then(() =>
			{
				...
				for (let parsedMedia of parsed.media)
				{
					...
					for (let ssrc of ssrcs.singles)
					{
						...
						if (!this._mapSingleSsrcToRtpReceivers.has(ssrc))
						{
							this._logger.debug('_handleRemoteReAnswer() | new receiving SSRC [ssrc:%s]', ssrc);
				...
				for (let ssrc of this._mapSingleSsrcToRtpReceivers.keys())
				{
					if (receivingSsrcs.indexOf(ssrc) === -1)
					{
						this._logger.debug('_handleRemoteReAnswer() | receiving SSRC removed [ssrc:%s]', ssrc);

						let rtpReceiver = this._mapSingleSsrcToRtpReceivers.get(ssrc);
	...
	_createRtpReceiverForSsrc(mediaSsrcs, parsedMedia)
	{
		...
		// Single stream.
		if (mediaSsrcs.length === 1)
		{
			// Store it in the map.
			this._mapSingleSsrcToRtpReceivers.set(mediaSsrcs[0], rtpReceiver);

			// When closed, remove from the map.
			rtpReceiver.on('close', () =>
			{
				this._mapSingleSsrcToRtpReceivers.delete(mediaSsrcs[0]);
			});
		}
		// TODO: simulcast.
		else
