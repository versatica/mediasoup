...
class RTCPeerConnectionPlanB extends RTCPeerConnectionCommon
{
	...
	_handleRemoteReOffer(remoteDescription)
	{
		...
		return Promise.resolve()
			...
			.then(() =>
			{
				...
				for (let parsedMedia of parsed.media)
				{
					...
					if (Array.isArray(parsedMedia.ssrcGroups))
					{
						for (let ssrcGroup of parsedMedia.ssrcGroups)
						{
							...
							receivingSsrcs.push(mediaSsrc);

							if (!this._mapRtpReceivers.has(mediaSsrc))
							{
								this._logger.debug('_handleRemoteReOffer() new receiving SSRC [ssrc:%s]', mediaSsrc);
				...
				{
					if (receivingSsrcs.indexOf(mediaSsrc) === -1)
					{
						this._logger.debug('_handleRemoteReOffer() receiving SSRC removed [ssrc:%s]', mediaSsrc);
