...
class Channel extends EnhancedEventEmitter
{
	...
	constructor({ socket, pid })
	{
		...
		this._socket.on('end', () => this._logger.debug('pipe ended by the worker process'));
		this._socket.on('error', (error) => this._logger.error('pipe error: %s', String(error)));
	}
