 ...
 * @return {Object}
 */
exports.createProducerRtpParametersMapping = function(params, caps)
{
	const codecPayloadTypeMap = {};
	...
	for (const codec of params.codecs)
	{
		const matchedCapCodec = caps.codecs
			.find((capCodec) =>
			{
				// TODO: This is not enough for RTX since there could be multiple RTX codecs
				// if simulcast.
				return matchCodec(codec, capCodec, true);
			});
		...
		codecPayloadTypeMap[codec.payloadType] = matchedCapCodec.preferredPayloadType;
	}
...
function matchCodec(codecA, codecB, isParamCodecA = false)
{
	if (!isParamCodecA)
	{
