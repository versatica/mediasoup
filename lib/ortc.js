...
/**
 * Translate Producer RTP parameters into the RTP capabilities of the Router. It
 * includes codec payload types, RTP header extension ids and ssrcs or (or rid
 * values translated into ssrcs).
 *
 ...
 *
 * @returns {Object} with:
 * - translatedRtpParameters {RTCRtpParameters}
 * - maping {Object} - Has codecs, headerExtensions and encodings arrays of objects.
 * @throws {TypeError} if wrong arguments.
 * @throws {UnsupportedError} if codec not supported.
 * @throws {Error}
 */
exports.translateProducerRtpParameters = function(params, caps)
{
	const translatedRtpParameters =
	{
		codecs           : [],
		headerExtensions : [],
		encodings        : [],
	...
	// Best effort to detect RTX support when there is RID so no encoding.rtx.ssrc.
	let hasRtx = false;
	...
	for (const codec of params.codecs)
	{
		...
		if (!capMediaCodec)
			throw new TypeError(`no media codec found for RTX PT ${codec.payloadType}`);
		...
		{
			throw new UnsupportedError(
				`no RTX codec found in capabilities [capabilities codec PT:${capMediaCodec.preferredPayloadType}]`);
		}
	...
	// Generate translated codecs and the corresponding mapping.
	for (const [ codec, capCodec ] of codecToCapCodec)
	{
		...
		if (codec.name.toLowerCase() !== 'rtx')
		{
			// NOTE: Let's keep capability codec rtcpFeedback as this will be useful
			// for Consumers.
			translatedCodec =
			{
				name         : codec.name,
			...
			if (!codec.channels)
				delete translatedCodec.channels;
		}
		// RTX codec.
		else
		{
			translatedCodec =
			{
				...
				{
					...codec.parameters,
					apt : capCodec.parameters.apt
				}
			};
		}

		translatedRtpParameters.codecs.push(translatedCodec);
	...
	// Match parameters header extensions to capabilities header extensions.
	...
	for (const ext of (params.headerExtensions || []))
	{
		...
		translatedRtpParameters.headerExtensions.push(
			{
				uri : ext.uri,
	...
	// Generate translated encodings.

	for (const encoding of (params.encodings || []))
	{
		const mappedSsrc = utils.randomNumber();
		const mappedRtxSsrc = hasRtx ? utils.randomNumber() : undefined;
		const translatedEncoding = utils.cloneObject(encoding);
		const mappedEncoding = {};

		// Remove rid field.
		delete translatedEncoding.rid;
		// Add translated ssrc.
		translatedEncoding.ssrc = mappedSsrc;
		// Add translated rtx.
		if (hasRtx)
			translatedEncoding.rtx = { ssrc: mappedRtxSsrc };

		translatedRtpParameters.encodings.push(translatedEncoding);
...
exports.getConsumableRtpParameters = function(params, caps, rtpMapping)
{
	...
	for (const codec of params.codecs)
	{
		...
		if (consumableCodecPt === undefined)
		{
			throw new TypeError(
		...
		if (!matchedCapCodec)
			throw new TypeError(`capabilities codec not found [name:${codec.name}]`);
	...
	for (const ext of params.headerExtensions || [])
	{
		...
		{
			throw new TypeError(
				`capabilities header extension not found [uri:${ext.uri}]`);
		}
