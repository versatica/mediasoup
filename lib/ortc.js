 ...
 * @return {Object}
 */
exports.adaptProducerRtpParametersToRtpCapabilities = function(params, caps)
{
	// Clone given parameters.
	...
	for (const codec of params.codecs)
	{
		...
		aptToCapApt[codec.parameters.apt] = associatedCapRtxCodec.parameters.apt;
		codecToCapCodec.set(codec, associatedCapRtxCodec);
	}

	const codecPayloadTypeMapping = new Map();

	for (const [ codec, capCodec ] of codecToCapCodec)
	{
		codecPayloadTypeMapping.set(codec.payloadType, capCodec.preferredPayloadType);
		codec.payloadType = capCodec.preferredPayloadType;

		if (codec.name.toLowerCase() === 'rtx')
			codec.parameters.apt = aptToCapApt[codec.parameters.apt];
	}

	const headerExtensionIdMapping = new Map();
	...
	for (const ext of params.headerExtensions)
	{
		...
		headerExtensionIdMapping.set(ext.id, matchedCapExt.preferredId);
		ext.id = matchedCapExt.preferredId;
	}

	return {
		adaptedRtpParameters : params,
		rtpMapping           :
		{
			codecPayloadTypes  : Array.from(codecPayloadTypeMapping),
			headerExtensionIds : Array.from(headerExtensionIdMapping)
		}
	};
};
