 ...
 *
 * NOTE: All the codecs of the consumable RTP parameters are copied verbatim
 * (all but RTX codecs in case the Peer does not support RTX). It's up to the
 * remote Consumer to check the codecs and decide whether it can enable this
 * Consumer or not.
 *
...
exports.getConsumerRtpParameters = function(consumableRtpParameters, rtpCapabilities)
{
	...
	consumerParams.codecs = consumableRtpParameters.codecs
		.filter((codec) =>
		{
			if (codec.name.toLowerCase() !== 'rtx')
				return true;

			const hasRtxCodec = rtpCapabilities.codecs
				.some((capCodec) => capCodec.preferredPayloadType === codec.payloadType);
