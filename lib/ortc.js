 ...
 * so the resulting consumable RTP parameters are suitable for a Consumer.
 * For now we assume a single encoding in the Producer RTP parameters.
 *
 * @param {RTCRtpParameters} params
 * @param {RTCRtpCapabilities} caps
...
exports.getConsumableRtpParameters = function(params, caps, rtpMapping)
{
	...
	// TODO: REMOVE
	// console.log('consumableParams:\n', JSON.stringify(consumableParams, null, '  '));
...
exports.getConsumerRtpParameters = function(consumableRtpParameters, rtpCapabilities)
{
	...
	for (const codec of consumableRtpParameters.codecs)
	{
		...
		if (!consumerCodec.channels)
			delete consumerCodec.channels;

		consumerParams.codecs.push(consumerCodec);

		if (consumerCodec.name.toLowerCase() === 'rtx')
			rtxSupported = true;
	}

	for (const ext of consumableRtpParameters.headerExtensions)
	{
		const matchedCapExt = rtpCapabilities.headerExtensions
			.find((capExt) => capExt.preferredId === ext.id);
		...
		consumerParams.headerExtensions.push(consumerExt);
	}

	const consumerEncoding =
	{
		ssrc : consumableRtpParameters.encodings[0].ssrc
	};
	...
	// TODO: REMOVE
	// console.log('consumerParams:\n', JSON.stringify(consumerParams, null, '  '));
