...
const logger = new Logger('RtpReceiver');
...
class RtpReceiver extends EventEmitter
{
	constructor(internal, data, channel)
	{
		...
		this._channel.on(this._internal.rtpReceiverId, (event, data2, buffer) =>
		{
			switch (event)
			{
				case 'close':
			...
			switch (event)
			{
				case 'rtpraw':
				{
					...
					this._channel.request(
						'rtpReceiver.setRtpRawEvent', this._internal, { enabled: true })
						.then(() =>
						{
				...
				case 'rtpobject':
				{
					// Send Channel request.
					this._channel.request(
						'rtpReceiver.setRtpObjectEvent', this._internal, { enabled: true })
						.then(() =>
						{
						...
						{
							logger.error(
								'"rtpReceiver.setRtpObjectEvent" request failed: %s', error);
						});
		...
		this.on('removeListener', (event) =>
		{
			switch (event)
			{
				case 'rtpraw':
				{
					...
					// Send Channel request.
					this._channel.request(
						'rtpReceiver.setRtpRawEvent', this._internal, { enabled: false })
						.then(() =>
						{
							logger.debug('"rtpReceiver.setRtpRawEvent" request succeeded');
						})
				...
				case 'rtpobject':
				{
					...
					// Send Channel request.
					this._channel.request(
						'rtpReceiver.setRtpObjectEvent', this._internal, { enabled: false })
						.then(() =>
						{
							logger.debug('"rtpReceiver.setRtpObjectEvent" request succeeded');
						})
	...
	close(error, dontSendChannel)
	{
		...
		if (!dontSendChannel)
		{
			...
			this._channel.request('rtpReceiver.close', this._internal)
				...
				.catch((error2) =>
				{
					logger.error('"rtpReceiver.close" request failed: %s', error2);
				});
		}
	...
	receive(rtpParameters)
	{
		...
		return this._channel.request(
			'rtpReceiver.receive', this._internal, rtpParameters)
			.then((data) =>
			{
