...
class Consumer extends EnhancedEventEmitter
{
	...
	{
		logger.debug('setPreferredProfile()');
		...
		this._channel.request('consumer.setPreferredProfile', this._internal, { profile })
			.then(() =>
			{
				logger.debug('"consumer.setPreferredProfile" request succeeded');

				this.emit('@notify', 'preferredProfileSet', data);
			})
			.catch((error) =>
			{
				logger.error('"consumer.setPreferredProfile" request failed: %s', String(error));
			});
	}
