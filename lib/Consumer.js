...
class Consumer extends EnhancedEventEmitter
{
	...
	enable(transport)
	{
		...
		const data = Object.assign({ transport }, this._data);
	...
	/**
	 * The associated source (Producer) has been paused.
	 *
	 * @private
	 *
	 * @param {Any} [appData] - App custom data.
	...
	sourcePause(appData)
	{
		logger.debug('sourcePause()');

		if (this._closed || this._sourcePaused)
			return;
	...
	resume(appData)
	{
		...
		if (this.enabled && !this._remotelyPaused)
		{
			// Send Channel request.
	...
	/**
	 * The associated source (Producer) has been resumed.
	 *
	 * @private
	 *
	 * @param {Any} [appData] - App custom data.
	 */
	sourceResume(appData)
	{
		logger.debug('sourceResume()');
