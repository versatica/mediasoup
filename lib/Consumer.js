...
class Consumer extends EnhancedEventEmitter
{
	...
	constructor({ internal, data, channel, appData, producerPaused })
	{
		...
		// Preferred spatial layer (just for video with simulcast or SVC).
		// @type {String}
		this._preferredSpatialLayer = null;

		// Effective spatial layer (just for video with simulcast or SVC).
		// @type {String}
	...
	/**
	 * Set preferred video spatial layer.
	 *
	 * @async
	 * @throws {TypeError} if invalid spatial layer.
	 */
	async setPreferredSpatialLayer(spatialLayer)
	{
		if (spatialLayer === this._preferredSpatialLayer)
			return;

		logger.debug('setPreferredSpatialLayer() [spatialLayer:%s]', spatialLayer);

		// if (!isValidSpatialLayer(spatialLayer))
		// 	throw new TypeError('invalid spatialLayer');

		const reqData = { spatialLayer };

		await this._channel.request(
			'consumer.setPreferredSpatialLayer', this._internal, reqData);
	...
	_handleWorkerNotifications()
	{
		this._channel.on(this._internal.consumerId, (event, data) =>
		{
			switch (event)
			{
				...
				case 'effectivespatiallayerchange':
				{
					const { spatialLayer } = data;

					this._effectiveSpatialLayer = spatialLayer;

					this.safeEmit('effectivespatiallayerchange', spatialLayer);
