...
class Consumer extends EnhancedEventEmitter
{
	constructor(internal, data, channel)
	{
		...
		this._channel.on(this._internal.consumerId, (event, data2) =>
		{
			switch (event)
			{
				case 'close':
				...
				{
					this._data.rtpParameters = data2.rtpParameters;
					this._data.active = data2.active;
					this.emit('parameterschange', data2.rtpParameters, data2.active);
					break;
				}
				...
				{
					this._data.active = data2.active;
					this.emit('activechange', data2.active);
					break;
				}
	...
	dump()
	{
		...
		return this._channel.request('consumer.dump', this._internal)
			...
			.catch((error) =>
			{
				logger.error('"consumer.dump" request failed: %o', error);
	...
	setTransport(transport)
	{
		...
		return this._channel.request('consumer.setTransport', this._internal)
			...
			.catch((error) =>
			{
				logger.error('"consumer.setTransport" request failed: %o', error);
	...
	enable(options)
	{
		...
		return this._channel.request('consumer.disable', this._internal,
			...
			.catch((error) =>
			{
				logger.error('"consumer.disable" request failed: %o', error);
