...
class Consumer extends EnhancedEventEmitter
{
	...
	get enabled()
	{
		return Boolean(this->transport) && Boolean(this->rtpParameters);
	}
	...
	enable(transport)
	{
		...
		return this._channel.request('router.createConsumer', this._internal, options)
			.then(() =>
			{
				...
				// On closure, the worker Transport closes all its Consumers and the client
				// side gets consumer.on('unhandled').
				transport.on('@close', () =>
				{
					this._internal.transportId = undefined;
					this._data.transport = undefined;
					this._data.rtpParameters = undefined;
	...
	send(rtpParameters)
	{
		...
		if (this._closed)
			...
			return Promise.reject(
				new errors.InvalidStateError('Consumer does not have Transport'));
		...
		return this._channel.request('consumer.send', this._internal, data)
			.then(() =>
			{
				logger.debug('"consumer.send" request succeeded');
			...
			.catch((error) =>
			{
				logger.error('"consumer.send" request failed: %s', String(error));
