...
class Room extends EventEmitter
{
	constructor(internal, data, channel)
	{
		...
		this._setCapabilities(data.rtpCapabilities);
	}
	...
	Peer(peerName, data)
	{
		logger.debug('Peer() [peerName:"%s", data:%o]',
			peerName, data);
		...
		if (!data.rtpCapabilities || typeof data.rtpCapabilities !== 'object')
			throw new TypeError('data.rtpCapabilities must be an object');
		...
		this._channel.request('room.createPeer', internal, data)
			.then(() =>
			{
