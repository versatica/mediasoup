	...
	constructor(internal, channel)
	{
		...
		this._channel.on(this._internal.roomId, (event, data) =>
		{
			switch (event)
			{
				case 'newrtpsender':
				{
					// TODO: get the receiver Peer, the sender Peer, create a RtpSender
					// instance and attach it to the sender Peer (by also passing the
					// receiver Peer) so he will emit 'newrtcsender' event with data
					// including the RtpSender instance and the receiver Peer
					// (and the receiver RtpReceiver too?)
					logger.warn('"newrtpsender" event: %o', data);
