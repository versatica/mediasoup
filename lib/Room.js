...
class Room extends EnhancedEventEmitter
{
			...
			switch (event)
			{
				case 'audiolevels':
				{
					...
					this._channel.request(
						...
						.catch((error) =>
						{
							logger.error('"room.setAudioLevelsEvent" request failed: %o', error);
						});
	...
	close(appData, notifyChannel = true)
	{
		...
		if (notifyChannel)
		{
			...
			this._channel.request('room.close', this._internal)
				...
				.catch((error) =>
				{
					logger.error('"room.close" request failed: %o', error);
				});
		}
	}
	...
	dump()
	{
		...
		return this._channel.request('room.dump', this._internal)
			...
			.catch((error) =>
			{
				logger.error('"room.dump" request failed: %o', error);
	...
	_createPeer(peerName)
	{
		...
		this._channel.request('room.createPeer', internal)
			...
			.catch((error) =>
			{
				logger.error('"room.createPeer" request failed: %o', error);
