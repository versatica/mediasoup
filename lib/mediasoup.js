...
process.on('unhandledRejection', (reason) =>
{
	if (reason instanceof Error)
		logger.error('"unhandledRejection" event due to a Promise being rejected with no error handler:\n%s', reason.stack);
	else
		logger.error('"unhandledRejection" event due to a Promise being rejected with no error handler [reason:%o]', reason);
});
