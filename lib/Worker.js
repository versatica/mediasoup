	...
	constructor({ child })
	{
		...
		this._channel.once(this._id, (event) =>
		{
			if (!spawnDone && event === 'running')
			{
				...
				logger.debug('worker process running [id:%s]', this._id);
		...
		this._child.on('exit', (code, signal) =>
		{
			if (!spawnDone)
			{
				...
				{
					logger.error(
						'worker process failed due to wrong settings [id:%s]', this._id);
				...
				{
					logger.error(
						'worker process failed unexpectedly [id:%s, code:%s, signal:%s]',
						this._id, code, signal);

					this.emit(
						'@failure',
						new Error(`[id:${this._id}, code:${code}, signal:${signal}]`));
				}
			}
			else
			{
				logger.error(
					'worker process died unexpectedly [id:%s, code:%s, signal:%s]',
					this._id, code, signal);
		...
		this._child.on('error', (error) =>
		{
			if (!spawnDone)
			{
				...
				logger.error(
					'worker process failed [id:%s]: %s', this._id, error.message);
			...
			{
				logger.error(
					'worker process error [id:%s]: %s', this._id, error.message);
