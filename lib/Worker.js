...
// Set an env variable to tell childs which the Control FD is
process.env.MEDIASOUP_CONTROL_FD = String(CONTROL_FD);
...
class Worker extends EventEmitter
{
	constructor(id, parameters)
	{
		...
		// Control unix socket
		this._control = this._child.stdio[CONTROL_FD];
	...
	createRoom()
	{
		...
		let roomId = randomString({ numeric: false, length: 8 }).toLowerCase();
		let req =
		{
			type   : 'request',
			method : 'createRoom',
		...
		this._control.write(JSON.stringify(req));
	}
}
