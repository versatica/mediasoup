...
const WORKER_BIN_PATH = path.join(__dirname, '..', 'worker', 'out', 'Release', 'mediasoup-worker');
const CHANNEL_FD = 3;
...
class Worker extends EventEmitter
{
	...
	Room(options)
	{
		...
		let room = new Room(internal, this._channel);

		// Store the Room instance and remove it when closed
		this._rooms.add(room);
		room.on('close', () => this._rooms.delete(room));

		this._channel.request('worker.createRoom', internal, options)
