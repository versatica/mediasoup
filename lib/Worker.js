...
class Worker extends EventEmitter
{
	...
	dump()
	{
		...
		return this._channel.request('worker.dump')
			...
			.catch((error) =>
			{
				logger.error('"worker.dump" request failed: %o', error);
	...
	updateSettings(options)
	{
		...
		return this._channel.request('worker.updateSettings', null, options)
			...
			.catch((error) =>
			{
				logger.error('"worker.updateSettings" request failed: %o', error);
	...
	Room(data)
	{
		...
		this._channel.request('worker.createRoom', internal)
			.then(() =>
			{
			...
			.catch((error) =>
			{
				logger.error('"worker.createRoom" request failed: %o', error);
