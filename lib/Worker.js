...
class Worker extends EnhancedEventEmitter
{
	...
	constructor({ id, child })
	{
		...
		// Map of Router instances indexed by id.
   ...
   *
	 * @promise
	 * @reject {InvalidStateError} if closed.
	 * @reject {Error}
	 */
   ...
   *
	...
	createRouter({ mediaCodecs })
	{
		...
		return Promise.resolve()
			...
			{
				// This may throw.
				rtpCapabilities = ortc.generateRouterRtpCapabilities(mediaCodecs);

				return this._channel.request('worker.createRouter', internal, null);
			})
			.then(() =>
			{
				const router =
				...
				router.on('@close', () => this._routers.delete(router.id));
