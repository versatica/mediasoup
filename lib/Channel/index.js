...
class Channel extends EventEmitter
{
	...
	request(method, data)
	{
		...
		if (Buffer.byteLength(ns) > NS_MAX_SIZE)
			return errors.TooBigPromise('message too big');
		...
		{
			return errors.Closed('Channel closed');
		}
		...
		promise = new Promise((pResolve, pReject) =>
		{
			let sent =
			{
				...
				timer: setTimeout(() =>
				{
					...
					pReject(errors.TimeoutError('Request timeout'));
				}, REQUEST_TIMEOUT),

				close: () =>
				{
					clearTimeout(sent.timer);
					pReject(errors.ClosedError('Channel closed'));
				}
			};
